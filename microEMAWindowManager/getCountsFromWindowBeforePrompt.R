...
combinedWristEMA <- uEMA01AnsweredPrompts
...
for (i in 1:nrow(uEMA01AnsweredPrompts)){
  ...
  for (j in k:nrow(uEMA01WristCounts)){
    print(paste0("Starting with the second file index: ", j, " and k is: ", k))
    
    countsPickedRow <- uEMA01WristCounts[j,]
    ...
    if (timeDifference <= timeBefore & timeDifference >= 0){
      ...
      countsAverage = sum(tempDataFrame$COUNTS_MAGNITUDE_WRIST)
      print("Assign average as a new column")
      
      print(paste0("CountsAverage is: ", countsAverage))
      
      combinedWristEMA$COUNTS_AVERAGE_WRIST_BEFORE[i] <- countsAverage
      #tempAverages[i,] <- i
      k = j
